...
2. Keep purposeful hard edges and flat facets. Quads and n-gons may remain in the author mesh: PREFLIGHT reports them as a warning and EXPORT triangulates the evaluated mesh with `calc_loop_triangles()`. Apply a Triangulate modifier only when a deliberately fixed diagonal is part of the authored result.
...
For a selected-object delivery, freeze the active object's exact name into the contract and keep the export object separate from unrelated scene meshes. When Geometry Nodes or modifiers are present, inspect the evaluated dependency-graph mesh and use its final UV layer and per-polygon material identity as the export truth. Require the high-level workflow's source-evaluated to prepared to logical-SMD material audit to pass; equal slot counts alone are not evidence. Do not validate only the pre-modifier mesh.

## Create UVs And Materials

1. Unwrap around visible seams, scale islands consistently, and inspect painted/checker distortion.
2. Assign a real image to every exported material. Match the SMD material token to the final `.bmp` filename, using `material["goldsrc_texture_token"]` when the Blender material name intentionally differs.
3. Keep each physical MDL texture at multiples of 16 and no larger than 512 for the conservative Half-Life/Counter-Strike profile. For a source atlas larger than 512, declare a logical `large_textures` record and let the Extension generate aligned `512x512` tiles; a `2048x2048` author image defines sixteen possible tiles, while the MDL embeds only those referenced after UV clipping or by skin families.
...
5. Let EXPORT convert the logical PNG into an uncompressed 8-bit indexed BMP with 256 palette entries. Reserve blue index 255 only for Masked textures; non-Masked textures may use all 256 indices.
6. Keep the prepared Blender material on the logical PNG and set its explicit logical `.bmp` token. Treat the indexed BMP as a compiler artifact; do not rebind it to the prepared material.
...
Keep source PBR/procedural UVs separate from the GoldSrc UV. The GoldSrc UV must be active on the evaluated export mesh when SMD triangles are written and must also be the UV layer with `active_render=True` before `bpy.ops.object.bake`. Declare `texture_bake.uv_layer` with `require_active_render: true` so PREFLIGHT can reject a raw/evaluated mismatch. Bake the visible color into a file-backed logical PNG and keep that PNG on the prepared material. The Extension converts it to compiler BMPs and reports evaluated UV/material facts, but it does not choose a correct unwrap or material meaning.

For a strict no-baked-shadow request, first run a `DIFFUSE`/`COLOR`-only bake as a diagnostic. If it is still dark, the darkness is part of the material graph rather than direct/indirect scene lighting. Make a temporary copy of the source material, replace its Principled/Diffuse closures with Emission nodes driven by Base Color at Strength `1.0`, preserve shader mix factors, and bake `EMIT` to a separate file-backed image. Keep alpha/masked behavior as a separate explicit output decision because an Emission closure does not carry a Principled alpha socket by itself. The high-level preparation API performs a second Emission opacity bake for supported Principled Alpha and Mix Shader combinations after `goldsrc_modes` is explicit; unsupported closures and any material Node Group return `needs_decision`. Never replace the saved author material with the temporary emission graph.

For a logical atlas, keep the GoldSrc UV across the full `0..1` image. EXPORT crops only referenced bottom-origin `512` tiles, clips triangles that cross tile edges, triangulates the clipped polygons, and remaps each tile's UVs locally. Check `export_plan.json`: `declared` lists all possible tiles, `compiled` lists actual MDL textures, and `omitted_unused_large_tiles` lists skipped empty regions. Validate the indexed BMP and generated token for each compiled tile, then confirm the MDL contains only `512x512` records. Never add hidden anchor triangles to force unused tiles into the texture table. Reject more than 64 possible tiles for one contract.
...
5. Save the logical PNG, let EXPORT create the indexed BMP, and compare color range and visible brightness through the unified author/readback evidence.
...
- Treat `mesh.non_triangles` as an informational export-triangulation warning when no authored diagonal is required; do not edit a valid mesh solely to silence it.
...
For one ordinary static selection, provide every explicit preparation strategy to `export_selected_static(..., assurance="strict")`. The Extension analyzes and freezes the evaluated object, creates the static rig and v2 contract, persists all five reports, performs isolated readback twice internally, and generates the unified comparison. Inspect detailed UV/material or conversion evidence only when the summary reports a warning/failure. Never decompose this route or repeat a passing stage manually; compilation alone does not validate UV placement, brightness, flags, orientation, or tile seams.
