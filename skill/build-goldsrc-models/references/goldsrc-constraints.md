...
- Triangulate and apply scale before export.
...
- Keep each physical MDL texture width and height at multiples of 16 and no larger than 512. A logical atlas may exceed 512 only when it is divisible into `512x512` tiles and expands to no more than 64 tiles in one MDL; the atlas is never embedded as one oversized texture.
