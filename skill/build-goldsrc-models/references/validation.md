...
For a static selection prepared from modifiers, require `static_material_audit.status == pass` in PREFLIGHT and EXPORT. Compare source evaluated material identity and face/triangle counts, explicit old-slot to prepared-slot mappings, and logical SMD token triangle counts. A prepared-to-readback RGB match is not sufficient because both sides can share the same incorrect remap.
