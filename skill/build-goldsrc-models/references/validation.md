...
For a static selection prepared from modifiers, require `static_material_audit.status == pass` in PREFLIGHT and EXPORT. For schema 2, recompute the source-evaluated and prepared geometry/material SHA-256 signatures in addition to comparing material identity, face/triangle counts, explicit old-slot to prepared-slot mappings, and logical SMD token triangle counts. This detects same-count deformation, winding changes, and per-face material swaps after preparation. A prepared-to-readback RGB match is not sufficient because both sides can share the same incorrect remap. Schema 1 is legacy-readable but lacks these signature guarantees.
...
For animated round trips, bind each imported Action in turn and render start, quarter, middle, three-quarter, and end frames up to the bounded preview budget. Record Action, frame, path, PNG byte `sha256`, decoded RGBA `pixel_sha256`, model-region occupancy, and foreground luminance for each still. Use `pixel_sha256`, including image dimensions and RGBA bytes, for frame equality; PNG metadata or compression may change the byte hash without changing a pixel. Compose the five images as a labeled `3x2` contact sheet with captions outside the image area. Preserve each source PNG and the layout JSON, including each cell's `source_pixel_sha256`. Inspect the contact sheet first to locate timing, pose, axis, framing, or texture anomalies, then open suspicious originals at full resolution; a scaled overview cannot prove fine seams, brightness, contact, or penetration. Warn when an Action has varying channels but all sampled decoded-pixel hashes are identical. Before saving `mdl_roundtrip.blend`, rebind the first Action and restore its start/range so viewport playback works immediately. Pixel metrics prove that evidence is present; the agent must still inspect the images for the requested visible result.
...
Require round-trip Actions, five representative images, one labeled contact sheet per Action, decoded-pixel variation for animated Actions, no `.001` names or stale contact sheets after strict isolated readback, and a saved Blend with the first Action bound at its start frame. The two ROUNDTRIP runs owned internally by `assurance="strict"` may have different PNG byte hashes, but corresponding `pixel_sha256` values must match. Do not manually repeat another passing stage. Open the Blend and verify Space playback after any change to Action reconstruction.
