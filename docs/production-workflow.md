...
For a baked procedural or PBR material, add `texture_bake: {"uv_layer": "GoldSrcUV", "require_active_render": true}` to the contract. Set that layer as both the raw mesh active UV and `active_render` before baking, then verify the same names on the evaluated export surface. PREFLIGHT warns about an undeclared active/render mismatch and fails a declared bake assertion that does not match; this keeps EXPORT deterministic without assuming that every explicit named UV node uses Blender's active-render layer.

The Extension does not perform or infer the source material bake. `DIFFUSE` with `COLOR` only and direct/indirect disabled is a useful diagnostic, but it can retain AO or other darkening authored in the material color graph. For a strict no-baked-shadow request, duplicate the source material, replace its Principled/Diffuse closures with Strength-1 Emission, bake `EMIT` on the declared GoldSrc UV, save the image, and restore/use an ordinary image-backed export material before `PREFLIGHT` and `EXPORT`. Treat the Emission bake report and image comparison as author evidence; `texture_bake` remains a UV assertion only.

For an author image larger than 512 pixels, keep the author atlas at its source resolution and declare it under `large_textures`. The MDL contains only generated `512x512` indexed tiles. Confirm the export report's tile list, cross-tile triangle count, local UV bounds, embedded MDL texture dimensions, and texture count. When the evaluated reference exceeds a submodel budget, accept only the generated multi-body plan and its per-part geometry reports; do not copy a raw `smdcutpy.py` slice into the QC.
