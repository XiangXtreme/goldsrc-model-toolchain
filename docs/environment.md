...
- Extension ID `goldsrc_model_toolchain`, release `1.2.0`, API version `1`.
...
Build `goldsrc_model_toolchain-1.2.0-windows-x64.zip`, emit its `.sha256`, audit extraction through `scripts/audit_release_archives.py`, and upload both files to GitHub Release `v1.2.0`. Release archives must contain no cache, local metadata, Blender MCP files, Source 1/2 assets, BSP, VTF/VMT, DMX/VTA, historical scenes, or runtime artifacts.
